...
export function RestaurantHeader({
  ...
  return (
    <div>
      {/* Cover image */}
      <div className="relative h-48 sm:h-56 bg-surface-tertiary">
        <img
      ...
      {/* Restaurant info */}
      <div className="bg-white px-4 pt-4 pb-3 -mt-6 relative rounded-t-3xl">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-3">
          <div className="flex items-center gap-1 bg-surface-secondary rounded-lg px-2.5 py-1.5">
            <Star className="w-3.5 h-3.5 fill-warning text-warning" />
            <span className="text-xs font-bold text-text-primary">{rating.toFixed(1)}</span>
          </div>
          <div className="flex items-center gap-1 bg-surface-secondary rounded-lg px-2.5 py-1.5">
            <Clock className="w-3.5 h-3.5 text-text-secondary" />
            <span className="text-xs font-semibold text-text-primary">{formatDeliveryTime(deliveryTime)}</span>
          </div>
          <div className="flex items-center gap-1 bg-surface-secondary rounded-lg px-2.5 py-1.5">
            <Bike className="w-3.5 h-3.5 text-text-secondary" />
            <span className="text-xs font-semibold text-text-primary">{formatPrice(deliveryFee)}</span>
          </div>
          <div className="flex items-center gap-1 bg-surface-secondary rounded-lg px-2.5 py-1.5">
          ...
          {cuisineType && (
            <div className="flex items-center gap-1 bg-surface-secondary rounded-lg px-2.5 py-1.5">
              ...
              <span className="text-xs font-semibold text-text-primary">{cuisineType}</span>
            </div>
          )}
        </div>
      </div>
