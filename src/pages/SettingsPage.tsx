    ...
    <div className="min-h-screen bg-background">
      ...
      {sections.map((section, si) => (
        <FadeIn key={section.title} delay={si * 0.1}>
          <div className="mt-2">
            <p className="px-4 py-2 text-xs font-bold text-white/40 uppercase tracking-wider">
              {section.title}
            </p>
              ...
              {section.items.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3.5 border-b border-white/10 last:border-none"
                  >
                    ...
                    {item.toggle ? (
                      <div className={`w-12 h-7 rounded-full transition-colors relative ${
                        item.isOn ? 'bg-primary' : 'bg-border-strong'
                      }`}>
                        <motion.div
